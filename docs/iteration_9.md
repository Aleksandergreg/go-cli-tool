...
- Fixed replay hint cleanup, malformed negative hint progress, quoted in-mission navigation, campaign-scoped totals, duplicate Docker play probes, and slow live Docker checks in `profile`.
- Bounded expanded shell text, arguments, pipeline stages, and total dispatches; each command line is expanded once and rejected before mutation when a preflight ceiling is exceeded.
...
- Made Docker teardown seal gameplay immediately while retaining and retrying only unresolved owned resources, including resources from an ambiguously failed create.
...
- Existing embedded mission JSON remains valid. Catalog loading now additionally enforces the existing beginner/intermediate/advanced vocabulary, one to five hints, condition-specific fields, and per-mission Docker fixture ceilings of 16 images and 32 containers.
