...
1. Virtual `mv` has two correctness bugs.
   ...
   - Moving a regular file onto the same resolved path deletes it. The destination is removed before descendants are collected in [filesystem.go](../../../internal/sandbox/filesystem.go#L280).
   - The current-directory protection is mistakenly in `cp` at [commands_files.go](../../../internal/sandbox/commands_files.go#L193), while `mv` has no protection. Therefore `cp -r . /out` is rejected, but `mv . /out` can leave `CWD`, `PWD`, and `OLDPWD` pointing at nonexistent paths.
...
2. Script resource limits can be bypassed.
   ...
   The 1 MiB script-output limit is checked after a line finishes in [commands_script.go](../../../internal/sandbox/commands_script.go#L117), but redirected output is already written and cleared in [shell.go](../../../internal/sandbox/shell.go#L140). `cat x >> x` can therefore double a virtual file repeatedly until the Go process runs out of memory.
...
3. Completionist can unlock prematurely.
   ...
   [session.go](../../../internal/game/session.go#L226) compares `len(Player.Completed)` with the current mission count. Removed or renamed mission IDs still count. The catalog-aware reconciliation in [app.go](../../../internal/cli/app.go#L64) does this correctly.
...
5. Blank-line processing is incorrect.
   ...
   [textLines](../../../internal/sandbox/commands_text.go#L499) treats a file containing exactly `"\n"` as having no lines. This affects `head`, `grep '^$'`, `sort`, `uniq`, `awk`, and `cut`.

6. Failed `tar -c` can leave phantom archive metadata.

   [commands_system.go](../../../internal/sandbox/commands_system.go#L241) publishes archive metadata before successfully creating the backing virtual file. Preflight and write first, then publish metadata.

## Architectural refactors

The most valuable architectural change before Docker is a narrow lab boundary. Currently:

- `Session` constructs a concrete sandbox in [session.go](../../../internal/game/session.go#L40).
- Terminal completion accepts `*sandbox.Sandbox` in [input.go](../../../internal/game/input.go#L52).
- Validators read sandbox internals directly in [validator.go](../../../internal/game/validator.go#L106).
...
- Replace the dispatcher, command-name list, and manual map in [shell.go](../../../internal/sandbox/shell.go#L325) with one command registry. It can describe help, script safety, stdin support, and interactive behavior.
...
- Save the profile once on a completion command. It is currently synchronously written at [session.go](../../../internal/game/session.go#L197) and again at line 230.
...
Profile persistence also needs explicit migrations and invariant validation rather than simply bumping every old version in [profile.go](../../../internal/profile/profile.go#L62). Concurrent OpsQuest processes are currently last-writer-wins.
...
One smaller CLI bug: in-mission `list --campaign "First Day"` cannot work because [missionNavigationFields](../../../internal/game/session.go#L257) uses `strings.Fields`, which ignores shell quoting.
