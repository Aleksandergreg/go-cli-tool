...
selected_route_output="$(printf 'mkdir -p reports/daily\ntouch reports/daily/summary.txt\nquit\n' | run_route_opsquest play 4)"
assert_contains "selected route" "${selected_route_output}" "Continuing to Mission 05: The Missing Log File"
assert_contains "selected route" "${selected_route_output}" "MISSION 05: The Missing Log File"
...
assert_contains "map" "${list_output}" "Stage 6/6 · #06"
assert_contains "map" "${list_output}" "0/23 missions complete"
...
assert_contains "show" "${show_output}" "World 1/4: First Day · Stage 1/6"
...
docker_show_output="$(run_opsquest show 20)"
assert_no_ansi "docker show" "${docker_show_output}"
assert_contains "docker show" "${docker_show_output}" "MISSION 20: Container Census"
...
assert_contains "in-mission list" "${play_output}" "1/23 missions complete"
assert_contains "in-mission switch" "${play_output}" "Switching to Mission 04: A Place for Everything"
assert_contains "in-mission switch" "${play_output}" "MISSION 04: A Place for Everything"

world_output="$(printf 'play 4\nquit\n' | run_opsquest play --world 2)"
assert_no_ansi "world jump" "${world_output}"
assert_contains "world jump" "${world_output}" "MISSION 07: Permission to Deploy"
assert_contains "world jump" "${world_output}" "World 2/4: The Logpocalypse · Stage 1/6"
assert_contains "world stage jump" "${world_output}" "Switching to Mission 10: The Runaway Worker"
assert_contains "world stage jump" "${world_output}" "World 2/4: The Logpocalypse · Stage 4/6"
assert_not_contains "world stage jump" "${world_output}" "MISSION 04: A Place for Everything"
