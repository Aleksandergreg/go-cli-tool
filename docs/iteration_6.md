...
1. Virtual `mv` has two correctness bugs.
   ...
   - Moving a regular file onto the same resolved path deletes it. The destination is removed before descendants are collected in [filesystem.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/filesystem.go:280).
   - The current-directory protection is mistakenly in `cp` at [commands_files.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/commands_files.go:193), while `mv` has no protection. Therefore `cp -r . /out` is rejected, but `mv . /out` can leave `CWD`, `PWD`, and `OLDPWD` pointing at nonexistent paths.
...
2. Script resource limits can be bypassed.
   ...
   The 1 MiB script-output limit is checked after a line finishes in [commands_script.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/commands_script.go:117), but redirected output is already written and cleared in [shell.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/shell.go:140). `cat x >> x` can therefore double a virtual file repeatedly until the Go process runs out of memory.
...
3. Completionist can unlock prematurely.
   ...
   [session.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/game/session.go:226) compares `len(Player.Completed)` with the current mission count. Removed or renamed mission IDs still count. The catalog-aware reconciliation in [app.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/cli/app.go:64) does this correctly.
...
5. Blank-line processing is incorrect.
   ...
   [textLines](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/commands_text.go:499) treats a file containing exactly `"\n"` as having no lines. This affects `head`, `grep '^$'`, `sort`, `uniq`, `awk`, and `cut`.

6. Failed `tar -c` can leave phantom archive metadata.

   [commands_system.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/commands_system.go:241) publishes archive metadata before successfully creating the backing virtual file. Preflight and write first, then publish metadata.

## Architectural refactors

The most valuable architectural change before Docker is a narrow lab boundary. Currently:

- `Session` constructs a concrete sandbox in [session.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/game/session.go:40).
- Terminal completion accepts `*sandbox.Sandbox` in [input.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/game/input.go:52).
- Validators read sandbox internals directly in [validator.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/game/validator.go:106).
...
- Replace the dispatcher, command-name list, and manual map in [shell.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/sandbox/shell.go:325) with one command registry. It can describe help, script safety, stdin support, and interactive behavior.
...
- Save the profile once on a completion command. It is currently synchronously written at [session.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/game/session.go:197) and again at line 230.
...
Profile persistence also needs explicit migrations and invariant validation rather than simply bumping every old version in [profile.go](/Users/aleksandergregersen/privat/go-cli-tool/internal/profile/profile.go:62). Concurrent OpsQuest processes are currently last-writer-wins.
...
One smaller CLI bug: in-mission `list --campaign "First Day"` cannot work because [missionNavigationFields](/Users/aleksandergregersen/privat/go-cli-tool/internal/game/session.go:257) uses `strings.Fields`, which ignores shell quoting.
