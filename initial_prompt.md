...
Each attempt maps stable logical aliases such as `api` to generated engine names and exact returned container IDs. Resources are labeled with the mission and a cryptographically random session identifier, run without a network as a non-root user with a read-only filesystem, dropped capabilities, `no-new-privileges`, and bounded CPU, memory, process, and file-descriptor limits. Cleanup verifies ownership labels before removing exact IDs and is idempotent across completion, quit, restart, switching, setup failure, and ordinary errors. Labs never use privileged mode, host bind mounts, host namespaces, devices, or a mounted Docker socket.

Docker mission validation observes only resources owned by the current attempt. Container Census requires both `api` and `metrics` to be running and exactly two attempt containers to exist, so an unnecessary replacement cannot satisfy the objective. Existing profile version 2 remains compatible: mission completion maps accept the new stable ID, while Linux percentages and Linux Completionist continue to consider only the 16 Linux missions.
